...
import { paymentApi, bookApi } from '../api/axiosClient';
...
function Payment() {
  ...
  const bookTotal = parseFloat(localStorage.getItem('bookTotal')) || 0;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (paymentResponse.data) {
        // Cập nhật status của book thành PAID
        await bookApi.patch(`/api/orders/${currentBook.id}`, { status: 'PAID' });

        setSuccess(true);

        // Xóa dữ liệu từ localStorage
        localStorage.removeItem('currentBook');
        localStorage.removeItem('bookTotal');
        ...
        <div className="w-full max-w-md text-center">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            <div className="mb-4 pb-4 border-b border-gray-200">
              ...
              <p className="text-lg font-bold text-brand-green">{currentBook.id}</p>
            </div>

            <div className="mb-4 pb-4 border-b border-gray-200">
              <p className="text-sm text-gray-600">Tổng tiền</p>
              <p className="text-2xl font-bold text-gray-800">
                {Math.round(bookTotal).toLocaleString('vi-VN')}₫
              </p>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 sticky top-20">
            ...
            <div className="space-y-3 pb-4 border-b border-gray-200">
              <div className="flex justify-between text-gray-700">
                <span>Mã đơn:</span>
                <span className="font-mono text-sm text-brand-green">{currentBook.id}</span>
              </div>
              <div className="flex justify-between text-gray-700">
                <span>Số mục:</span>
                <span className="font-semibold">{currentBook.movieIds?.length || 0}</span>
              </div>
            </div>
            ...
            <div className="my-6 pb-6 border-b border-gray-200">
              <div className="flex justify-between text-gray-700 mb-2">
                <span>Giá tiền:</span>
                <span className="font-semibold">{Math.round(bookTotal * 0.926).toLocaleString('vi-VN')}₫</span>
              </div>
              <div className="flex justify-between text-gray-700">
                <span>Phí giao + Thuế:</span>
                <span className="font-semibold">{Math.round(bookTotal * 0.074).toLocaleString('vi-VN')}₫</span>
              </div>
            </div>
